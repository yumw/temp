  ...
  <div>
    <el-form :model="formAddEdit" :rules="formAddEditRules" ref="formAddEdit" label-width="120px" class="add-edit-wrap">
      ...
      <el-form-item label="资方名称" prop="partnerName">
        ...
        <el-autocomplete
          v-model="formAddEdit.partnerName"
      ...
      <el-form-item label="合作模式" prop="cooperativeMode">
        <el-select v-model="formAddEdit.cooperativeMode" auto-complete="off" filterable @change="cooperativeModeChange">
          <el-option v-for="item in cooperativeMode" :key="item.value" :label="item.label" :value="item.value"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="中邮出资比例" prop="investRatio">
        <el-col :span="20">
          <el-input type="number" v-model="formAddEdit.investRatio" :disabled="formAddEdit.cooperativeMode!='1'" auto-complete="off" oninput="value = /^(\d{1,2}|100)$/.test(value) ? value : ''" ></el-input>
        </el-col>
      ...
      <el-form-item label="审批时效" prop="approveTime">
        <el-select v-model="approveTime.h" @change="changeApproveTime">
          <el-option v-for="item in 25" :key="item" :value="item.toString().length < 2 ? '0'+(item-1) : (item-1)"></el-option>
        </el-select>
        <span>时</span>
        <el-select v-model="approveTime.m">
          <el-option v-if="approveTime.h == '24'" :key="1" value="00"></el-option>
          <el-option v-else v-for="item in 60" :key="item" :value="item.toString().length < 2 ? '0'+(item-1) : (item-1)"></el-option>
        </el-select>
        <span>分</span>
        <el-select v-model="approveTime.s">
          <el-option v-if="approveTime.h == '24'" :key="1" value="00"></el-option>
          <el-option v-else v-for="item in 60" :key="item" :value="item.toString().length < 2 ? '0'+(item-1) : (item-1)"></el-option>
        </el-select>
        <span>秒</span>
      ...
      <el-form-item>
        <el-checkbox v-model="advanceCloseLongFlag" @change="changeAdvanceCloseLongFlag">长期有效</el-checkbox>
      </el-form-item>  
      </div>
    ...
    <packageCode-modal ref="packageCodeModal" :list="formAddEdit.packageCode" @confirm="getpackageCode"></packageCode-modal>
  </div>
</template>
<script>
...
export default  {
  ...
  mounted(e){
    ...
    if(record){
      ...
      this.advanceCloseLongFlag = record.advanceCloseLongFlag == 1 ? true : false;
      if(this.advanceCloseLongFlag === true){
