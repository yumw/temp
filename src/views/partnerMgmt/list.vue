  ...
  <div class="app-container">
    ...
    <div class="table-container">
      ...
      <el-table ref="multipleTable" :data="tableData" border :stripe="stripe" style="width: 100%">
        ...
        <el-table-column prop="approveTime" label="审批时效"></el-table-column>
        <el-table-column prop="advanceFlag" label="提现开关" width="330">
          <template slot-scope="scope">
            ...
            <div v-if="scope.row.advanceFlag === 'N'">
              关
              <span v-if="scope.row.advanceCloseLongFlag === 'Y'">长期</span>
              <span
                v-if="scope.row.advanceCloseLongFlag === 'N'"
              >{{formatTime(scope.row.advanceCloseBeginTime)}} - {{formatTime(scope.row.advanceCloseEndTime)}}</span>
            </div>
          </template>
        </el-table-column>
