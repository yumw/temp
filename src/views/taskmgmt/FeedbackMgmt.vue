  ...
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table ref="multipleTable" :data="tableData" border :stripe="stripe" style="width: 100%">
        ...
        <el-table-column prop="state" label="状态" :formatter="translate"></el-table-column>
        <!-- <el-table-column prop="failReason" label="失败原因"></el-table-column> -->
        <el-table-column prop="createTime" label="创建时间"></el-table-column>
        <!-- <el-table-column prop="updateTime" label="更新时间"></el-table-column> -->
        <el-table-column label="操作" width="180" fixed="right">
          <template slot-scope="scope">
            <el-button @click="check(scope.row.id, scope.row.replyType)" type="primary" size="mini">查看</el-button>
            <el-button v-if="canRetry(scope.row.serviceName)&&scope.row.scource === '服务开放前置'" @click="retry(scope.row)" type="danger" size="mini">重试</el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
import { replyMgmtQry, replyMgmtQryDetail, replyRetry } from '@/api/taskmgmt'
...
export default {
  ...
  methods: {
    query(pageSize, pageNum) {
      ...
      replyMgmtQry(params).then((response) => {
    ...
    check(id, replyType) {
      replyMgmtQryDetail({ id, replyType }).then((response) => {
        console.log(response.data)
        this.$store.dispatch('SetValues', { requestDetails: response.data })
        this.$router.push('TaskDetails/feedback')
      }).catch(error => {
        console.log(error)
      })
    },
    retry(row) {
      replyRetry({ serialNumber: row.serialNumber, id: row.id }).then((response) => {
